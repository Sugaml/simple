...
func TestFindPaymentSettingByID(t *testing.T) {
	paymentsetting := PaymentSetting{
		...
		Street:      "Lainchour",
		Postal_Code: "44600",
		Promocode:   "Newyearoffer",
	}
	paymentsetting.ID = 1
	fmt.Println(paymentsetting)
	server.Mock.ExpectQuery(regexp.QuoteMeta(`SELECT`)).WillReturnRows(sqlmock.NewRows([]string{"id", "user_id", "country", "state", "city", "street", "postal_code", "promocode", "CreatedAt", "UpdatedAt"}).AddRow(1, 1, "Nepal", "Bagmati", "Kathmandu", "Lainchour", "44600", "Newyearoffer", time.Now(), time.Now()))
	server.Mock.ExpectCommit()
	server.Mock.MatchExpectationsInOrder(false)
